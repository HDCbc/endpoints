...
import:
	...
	@	. ./config.env; \
		...
		sudo docker run --rm --name e2e-oscar -h e2e-oscar -e DEL_DUMPS="$${DEL_DUMPS}" --link gateway --volume "$${SQL_PATH}":/import:rw --cpus='2.5' --memory=1280m --net docker_default hdcbc/e2e_oscar:"${TAG}"; \
		TIME_AFTER=$$( date +%s ); \
